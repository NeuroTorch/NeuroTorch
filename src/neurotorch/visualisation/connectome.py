...
def visualize_init_final_weights(
    ...
    **kwargs,
    ...
    initial_wights, final_weights = nt.to_numpy(initial_wights), nt.to_numpy(
        final_weights
    )
    ...
    if fig is None:
        ...
        assert isinstance(
            axes, Iterable
        ), "If fig is not None, axes must be an iterable"
    ...
    if kwargs.get("cmp_zero_center", True):
        ...
        if (
            not np.isclose(min_value, max_value)
            and min_value < max_value
            and min_value < 0 < max_value
        ):
            divnorm = colors.TwoSlopeNorm(vmin=min_value, vcenter=0.0, vmax=max_value)
    ...
    if kwargs.get("compute_dale", "dale_law_kwargs" in kwargs):
        ...
        initial_dale = nt.to_numpy(
            nt.DaleLaw([nt.to_tensor(initial_wights)], **dale_law_kwargs)()
        )
        final_dale = nt.to_numpy(
            nt.DaleLaw([nt.to_tensor(final_weights)], **dale_law_kwargs)()
        )
