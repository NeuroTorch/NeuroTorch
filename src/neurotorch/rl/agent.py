...
class Agent(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        continuous_action_mid_ranges = {
            k: to_tensor(self.action_spec[k].high - self.action_spec[k].low) / 2 for k in self.continuous_actions
        }
    ...
    def get_actions(
        ...
        **kwargs,
        ...
        re_actions_list = [
            self.format_batch_discrete_actions(out_actions, re_format=re_format) for re_format in re_formats
        ]
        if as_numpy:
        ...
        **kwargs,
        ...
        if re_format.lower() in ["logits", "raw"]:
            ...
            if isinstance(actions, torch.Tensor):
                ...
                return {
                    k: (torch.softmax(v, dim=-1) if (k in discrete_actions or len(continuous_actions) == 0) else v)
            ...
            if isinstance(actions, torch.Tensor):
                ...
                return {
                    k: (torch.log_softmax(v, dim=-1) if (k in discrete_actions or len(continuous_actions) == 0) else v)
                    for k, v in actions.items()
                }
            else:
                raise ValueError(f"Cannot format actions of type {type(actions)}.")
            ...
            if isinstance(actions, torch.Tensor):
                ...
                return {
                    k: (
                        torch.argmax(v, dim=-1).long() if (k in discrete_actions or len(continuous_actions) == 0) else v
                    )
                    for k, v in actions.items()
                }
            else:
            ...
            if isinstance(actions, torch.Tensor):
                ...
                return {
                    k: (torch.max(v, dim=-1).values if (k in discrete_actions or len(continuous_actions) == 0) else v)
                    for k, v in actions.items()
                }
            else:
                raise ValueError(f"Cannot format actions of type {type(actions)}.")
