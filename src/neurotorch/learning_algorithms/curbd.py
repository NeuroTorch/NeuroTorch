...
class CURBD(LearningAlgorithm):
	...
	def _batch_step(self, trainer, **kwargs):
		...
		phiPphi = [K[i] @ pred_batch_view.T for i in range(len(self.params))]  # (B, m) @ (m, B) -> (B, B)
		...
		for param, k in zip(self.params, K):
			param.data -= (c * error @ k.T).to(param.device, non_blocking=True).view(param.data.shape)
