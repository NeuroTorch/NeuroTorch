...
class Eprop(TBPTT):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(params=params, layers=layers, output_layers=output_layers, **kwargs)
    ...
    def initialize_feedback_weights(
        ...
        self.feedback_weights = {
            k: [self.make_feedback_weights(y_batch_item.shape[-1], pld, generator=self.rn_gen) for pld in last_dims]
    ...
    def initialize_output_params(self, trainer=None):
        ...
        if not self.output_params:
            self.output_params = [param for layer in self.output_layers for param in layer.parameters()]
        if not self.output_params:
    ...
    def compute_errors(
        ...
        if isinstance(y_batch, dict) or isinstance(pred_batch, dict):
            ...
            batch_errors = {k: (pred_batch[k] - y_batch[k].to(pred_batch[k].device)) for k in y_batch}
        else:
