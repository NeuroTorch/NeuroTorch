...
class WilsonCowanLayerDebug(BaseNeuronsLayer):
	"""
	TODO: DELETE THIS AFTER DEBUGGING
	Forward pass was simplified for debugging purposes.
	"""
	...
	def __init__(
		...
		Remarks: Parameter mu and r can only be a parameter as a vector.
		"""
		super(WilsonCowanLayerDebug, self).__init__(
			input_size=input_size,
			output_size=output_size,
			use_recurrent_connection=use_recurrent_connection,
			dt=dt,
	...
	def forward(
		...
		if self.use_recurrent_connection:
			rec_inputs = torch.matmul(inputs, torch.mul(self.recurrent_weights, self.rec_mask))
		else:
			rec_inputs = 0.0
