...
class WilsonCowanDynamic(nn.Module):
	...
	def __init__(
		...
		self.device = device
		self.dt = dt
		assert r >= 0.0, f"r must be positive"
...
def train_with_params(
	...
	#optimizer_regul = torch.optim.SGD([model.forward_weights], lr=1e-3)
	...
	for epoch in progress_bar:
		...
		#print(f"epoch {epoch}/{epochs} pVar {loss.item():.4f} MSE {mse_loss.detach().item():.4f}")
	out = {}
