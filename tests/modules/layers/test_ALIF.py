...
from neurotorch.modules.layers import ALIFLayer, LearningType
...
class TestALIFLayer(unittest.TestCase):
	...
	def test_kwargs_specified_parameter(self):
		...
		self.assertTrue(np.isclose(layer.kwargs["gamma"], 72.0), f"{layer.kwargs['gamma'] = } is not equal to 100")
	...
	def test_backward(self):
		...
		layer = ALIFLayer(
			...
			learn_beta=True, spike_func=HeavisidePhiApprox
		)
		input_ = torch.rand(1, 3)
		output = layer(input_)[0]
		output.mean().backward()
